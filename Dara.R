...
main<-function(){
  ...
  plot_attacks_by_cntry <- after_1997%>%
    count(country_txt)%>%
    arrange(n)%>%
    hchart(type = "treemap", hcaes(x = country_txt, value = n, color = n))%>% 
      #hc_colorAxis(stops = color_stops(colors = viridis::inferno(10)))%>%
  hc_title(text = "Killed by country") %>% 
  #print(plot_attacks_by_cntry)
    
  plot_attacks_by_region <- after_1997%>%
    count(region)%>%
    arrange(n)%>%
    hchart(type = "treemap", hcaes(x = region, value = n, color = n))%>% 
    #hc_colorAxis(stops = color_stops(colors = viridis::inferno(10)))%>%
    hc_title(text = "Killed by country") %>% 
    #print(plot_attacks_by_cntry)

    
  after_1997 %>% group_by(iyear,region_txt) %>% summarise(nkills = sum(nkill)) %>% ungroup() -> dfyr
