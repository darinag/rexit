...
main<-function(){
  ...
  # # # # # # # # # # # # # # # # # # # # # EUROPE  # # # # # # # # # # # # # # # # # # # # # # # # # # # #  # # # # # # # # # # # # # # # # # # # # #
  europe <- subset(loans, (Region==9 | Region==8 | Region==10))
  europe <- subset(europe, (Attack_Type != 9))
  europe <- replace.value(europe, c("Weapon_Type"), from=13, to=as.double(12))
  # europe <- replace.value(europe, c("Weapon_Type"), from=c("Unknown"), to=c("Other"))
  
  #countries<- loans %>% group_by(Country) %>% summarize(count=n())
  regions<- europe %>% group_by(Region) %>% summarize(count=n())
  attack_type <- europe %>% group_by(Attack_Type) %>% summarize(count=n())
  weapon_type <- europe %>% group_by(Weapon_Type) %>% summarize(count=n())
  ...
  ordinals<-loans[,which(field_types1==TYPE_DISCREET | field_types1==TYPE_ORDINAL)]
  ...
  data_train_matrix <- as.matrix(scale(combinedML))
